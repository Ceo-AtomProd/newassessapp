...
export default function Index() {
  ...
  const filterOptions = [ {value:-1,text:"New to Old"},{value:1,text:"Old To New"}];
  ...
  const fetchData = async () => {
    try {
      await axios({
        method: "get",
        url: `/api/user/assess?page=${page}&search=${search}&filter=${filter}`,
      }).then(function ({ data }) {
        setTimeout(() => {
  ...
  const setFunc = (id) => {
    setLinkData(allAssess.find((item) => item._id === id));
  };
                  ...
                  {allAssess?.map((i) => (
                    <tr className="border-b" key={i?._id}>
                      ...
                      <td className="px-4 py-3 text-gray-700">
                        <Link href={`/assess/${i.assessId._id}`}>
                       {i.assessId.title}</Link>
                      </td>
                      ...
                      <td className="px-4 py-3 text-gray-700">
                        {i.isComplete ? `${i.testresult.score}-(${i.testresult.status})`  : "Not complete yet"}
                      </td>
                      <td className="px-4 py-3 flex items-center justify-center gap-3">
                      <button
                          ...
                          onClick={() => {
                            ...
                            tempInput.value = `${process.env.NEXT_PUBLIC_WEB_URL}/tests/assess/${i._id}`;
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M14 8H4c-1.103 0-2 .897-2 2v10c0 1.103.897 2 2 2h10c1.103 0 2-.897 2-2V10c0-1.103-.897-2-2-2z"></path><path d="M20 2H10a2 2 0 0 0-2 2v2h8a2 2 0 0 1 2 2v8h2a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2z"></path></svg></button>
                        <button
