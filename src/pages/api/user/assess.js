    ...
    if (role === 'User') {
      if(req.method === 'POST') {
        ...
        if(assessId && name && dob){
         const assessRes = await new AssessResponse({
            assessId,
            name,
            dob,
            createrUserId:id
      ...
      if(req.method == "GET"){
        ...
        const assess = await AssessResponse.find(conditions).populate('assessId', 'title category')
          .skip(skip)
          .limit(pageSize)
      ...
      if (req.method === "DELETE") {
        try {
          ...
          if (_id) {
            await AssessResponse.deleteOne({_id})
