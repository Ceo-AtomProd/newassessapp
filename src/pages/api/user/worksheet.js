    ...
    if (role === 'User') {
      if(req.method === 'POST') {
        ...
        if(worksheetId && name && dob){
         const worksheetRes = await new WorksheetResponse({
            worksheetId,
            name,
            dob,
            createrUserId:id
      ...
      if(req.method == "GET"){
        ...
        const worksheet = await WorksheetResponse.find(conditions).populate('worksheetId', 'title category columns')
          .skip(skip)
          .limit(pageSize)
          .sort({ _id: Number(filter)})
          .exec()
        const totalWorksheet = await WorksheetResponse.countDocuments(conditions);
      ...
      if (req.method === "DELETE") {
        try {
          ...
          if (_id) {
            await WorksheetResponse.deleteOne({_id})
