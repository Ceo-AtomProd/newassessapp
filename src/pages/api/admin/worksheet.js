    ...
    if (role === "Admin") {
      if (req.method === "POST") {
        ...
        if (
          ...
          const createWorksheet = await Worksheet({
            title,
            description,
            category,
            image,
      ...
      if (req.method === "GET") {
        ...
        const worksheet = await Worksheet.find(conditions)
          .skip(skip)
          .limit(pageSize)
          .sort({ _id: -1 })
          .exec();
        const totalWorksheet = await Worksheet.countDocuments(conditions);
        res.status(200).json({
          success: true,
      ...
      if (req.method === "PUT") {
        try {
          ...
          if (_id &&
            ...
            const updateWorksheet = await Worksheet.updateOne({_id},{
              $set: {title,
              description,
              category,
              image,
